...
1. Deploy the Operator and associated resources:
   ...
   ```shell
   make deploy IMG=nginx/nginx-ingress-operator:3.7.0
   ```
...
`kubectl apply -f https://raw.githubusercontent.com/nginx/nginx-ingress-helm-operator/v3.7.0/resources/scc.yaml`

Alternatively, to create an SCC for NIC daemonsets, please run this command:

`kubectl apply -f https://raw.githubusercontent.com/nginx/nginx-ingress-helm-operator/v3.7.0/resources/scc-daemonset.yaml`

You can now deploy the NGINX Ingress Controller instances.

**Note: If you're upgrading your operator installation to a later release, navigate to the [Helm charts directory](../helm-charts/nginx-ingress/) and run `kubectl apply -f crds/` or `oc apply -f crds/` as a prerequisite**
...
1. Tag the images for your private registry
   ...
   ```shell
   docker tag quay.io/nginx/nginx-ingress-operator:3.7.0 <my-private-registry>/nginx-ingress-operator:3.7.0
...
2. Push the image to your private registry
   ...
   ```shell
   docker push <my-private-registry>/nginx-ingress-operator:3.7.0
...
3. Follow step 1 above but in step 1.2 you can run
   ...
   ```shell
   make deploy IMG=<my-private-registry>/nginx-ingress-operator:3.7.0 KRP_IMAGE_BASE=<my-private-registry>/kube-rbac-proxy
   ```
