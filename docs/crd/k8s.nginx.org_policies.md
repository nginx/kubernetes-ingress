...
| `cache.cacheKey` | `string` | CacheKey defines a key for caching (proxy_cache_key). By default, close to "$scheme$proxy_host$uri$is_args$args". |
...
| `cache.useTempPath` | `boolean` | UseTempPath controls whether temporary files and the cache are put on different file systems (use_temp_path parameter). If set to off, temporary files will be put directly in the cache directory. |
