...
| `upstreams[].proxy-http-version` | `string` | Sets the HTTP protocol version used for connections to the upstream servers. Supported values are "1.0", "1.1" and "2". If unset, the appProtocol of the backing Service is used ("kubernetes.io/h2c" implies "2"), otherwise "1.1". Note: "2" is ignored for upstreams with type "grpc". Requires NGINX 1.29.4+. Allowed values: `"1.0"`, `"1.1"`, `"2"`. |
