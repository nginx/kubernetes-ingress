...
- Optional: ```nginx.org/upstream-vhost: "hostname"``` - specifies the value of the `Host` header sent to the upstream

By default, NGINX Ingress Controller passes the `Host` header of the client request to the upstream, which is generated as
`proxy_set_header Host $host;`. The `nginx.org/upstream-vhost` annotation replaces that value, which is useful when the
upstream server must be addressed by a name other than the one used by the client.
...
In the following example, the ``nginx.org/upstream-vhost`` annotation is used to send `backend.example.com` as the `Host`
...
metadata:
  name: cafe-ingress
  annotations:
    nginx.org/upstream-vhost: "backend.example.com"
spec:
...
### Example 2: No Annotation

Without the annotation, the `Host` header of the client request is passed to the upstream:
...
location / {
  ...
  grpc_set_header Host backend.example.com;
