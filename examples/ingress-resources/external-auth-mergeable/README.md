# External Authentication with Mergeable Ingress (Basic Auth + OAuth2)

In this example we deploy a web application, configure load balancing using the Mergeable Ingress (master/minion)
pattern, and apply external authentication using two different methods on different routes:
...
Each minion Ingress references its own ExternalAuth Policy CRD (`k8s.nginx.org/v1 Policy`) via the `nginx.org/policies`
annotation. The controller automatically generates the required internal auth subrequest locations and upstreams -- no
...
For a simpler example that uses only basic-auth with a single Ingress, see [external-auth](../external-auth/).
...
Deploy the ExternalAuth policies that the minion Ingress resources will reference:
