...
| `make codegraph` | Regenerate `.codegraph/` static code index |
| `make codegraph-check` | Drift-check `.codegraph/` (CI + pre-commit) |
...
After Go, template, Helm, examples, or python-test edits, run `make codegraph`.

---

## Code Graph

A static index lives at [.codegraph/](.codegraph/), regenerated by `make codegraph` and validated in CI.
Read [.codegraph/INDEX.md](.codegraph/INDEX.md) first for *where* things live (symbols, callers, CRD wiring, templates, Helm value usage). Load the `nic-codegraph` skill for query patterns. Load `nic-structure` first when you need *why/how* — the index only answers *where*.
...
| `nic-codegraph` | Plan + Dev + Review | Looking up file:line for symbols, callers, CRD wiring, template/Helm/test references |
