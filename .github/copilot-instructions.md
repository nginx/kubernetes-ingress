...
| `make nic-graph` | Regenerate `.nic-graph/` static code index |
| `make nic-graph-check` | Drift-check `.nic-graph/` (CI + pre-commit) |

- Always load `nic-graph` skill even when delegating tasks to subagents. Use `grep/glob` as a fallback.
...
- After Go, template, Helm, examples, or python-test edits, run `make nic-graph`.

---

## NIC Code Graph

A static index lives at [.nic-graph/](.nic-graph/), regenerated by `make nic-graph` and validated in CI.
Read [.nic-graph/INDEX.md](.nic-graph/INDEX.md) first for *where* things live (symbols, callers, CRD wiring, templates, Helm value usage). Load the `nic-graph` skill for query patterns. Load `nic-structure` first when you need *why/how* — the index only answers *where*.
...
| `nic-graph` | Plan + Dev + Review | Looking up file:line for symbols, callers, CRD wiring, template/Helm/test references |
