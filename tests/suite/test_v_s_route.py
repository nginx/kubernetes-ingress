...
class TestVirtualServerRouteSelector:
    def test_responses_and_events_in_flow(
        ...
        vsr_1_name = f"{v_s_route_selector_setup.namespace}/{v_s_route_selector_setup.route_m.name}"
        ...
        print("\nStep 6: remove VSRoute and check")
        delete_v_s_route(
            kube_apis.custom_objects, v_s_route_selector_setup.route_m.name, v_s_route_selector_setup.namespace
        )
        wait_before_test(1)
        new_config = get_vs_nginx_template_conf(
            kube_apis.v1,
