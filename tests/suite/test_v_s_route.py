...
class TestVirtualServerRouteSelector:
    def test_responses_and_events_in_flow(
        ...
        vsr_name = f"{v_s_route_selector_setup.namespace}/{v_s_route_selector_setup.route_s.name}"
        vsr_event_text = f"Configuration for {vsr_name} was added or updated"
        vs_event_text = f"Configuration for {vs_name} was added or updated"
        # vs_warning_event_text = f"Configuration for {vs_name} was added or updated with warning(s): VirtualServerRoute {vsr_name} doesn't exist or invalid"
        initial_config = get_vs_nginx_template_conf(
            kube_apis.v1,
            v_s_route_selector_setup.namespace,
            v_s_route_selector_setup.vs_name,
        ...
        events_ns_1 = get_events(kube_apis.v1, v_s_route_selector_setup.route_s.namespace)
        assert resp_1.status_code == 200
        assert "Server name: backend1-" in resp_1.text
        assert_locations_in_config(initial_config, v_s_route_selector_setup.route_s.paths)
        initial_count_vsr_1 = assert_event_and_get_count(vsr_event_text, events_ns_1)
        initial_count_vs = assert_event_and_get_count(vs_event_text, events_ns_1)
        ...
        patch_v_s_route_from_yaml(
            kube_apis.custom_objects,
            v_s_route_selector_setup.route_s.name,
            f"{TEST_DATA}/virtual-server-route-selector/route-selector-single-updated.yaml",
        ...
        assert "Server name: backend1-" in resp_1.text
        events_ns_1 = get_events(kube_apis.v1, v_s_route_selector_setup.route_s.namespace)
        assert_event_and_count(vsr_event_text, initial_count_vsr_1 + 1, events_ns_1)
        assert_event_with_full_equality_and_count(vs_event_text, initial_count_vs + 1, events_ns_1)
        ...
        patch_v_s_route_from_yaml(
            kube_apis.custom_objects,
            v_s_route_selector_setup.route_s.name,
            f"{TEST_DATA}/virtual-server-route-selector/route-selector-single.yaml",
            v_s_route_selector_setup.route_s.namespace,
        )
        wait_before_test(1)
        resp_1 = requests.get(
        ...
        assert "Server name: backend1-" in resp_1.text
        events_ns_1 = get_events(kube_apis.v1, v_s_route_selector_setup.route_s.namespace)
        assert_event_and_count(vsr_event_text, initial_count_vsr_1 + 2, events_ns_1)
        assert_event_with_full_equality_and_count(vs_event_text, initial_count_vs + 2, events_ns_1)

        print("\nStep 4: update one backend service port and check")
        svc_1 = read_service(kube_apis.v1, "backend1-svc", v_s_route_selector_setup.route_s.namespace)
        svc_1.spec.ports[0].port = 8080
        replace_service(kube_apis.v1, "backend1-svc", v_s_route_selector_setup.route_s.namespace, svc_1)
        wait_before_test(1)
        resp_1 = requests.get(
            f"{req_url}{v_s_route_selector_setup.route_s.paths[0]}", headers={"host": v_s_route_selector_setup.vs_host}
        )
        assert resp_1.status_code == 502
        events_ns_1 = get_events(kube_apis.v1, v_s_route_selector_setup.route_s.namespace)
        assert_event_and_count(vsr_event_text, initial_count_vsr_1 + 3, events_ns_1)
        assert_event_with_full_equality_and_count(vs_event_text, initial_count_vs + 3, events_ns_1)
        ...
        assert resp_1.status_code == 200
        events_ns_1 = get_events(kube_apis.v1, v_s_route_selector_setup.route_s.namespace)
        assert_event_and_count(vsr_event_text, initial_count_vsr_1 + 4, events_ns_1)
        assert_event_with_full_equality_and_count(vs_event_text, initial_count_vs + 4, events_ns_1)

        # print("\nStep 6: remove VSRoute and check")
        # delete_v_s_route(kube_apis.custom_objects, v_s_route_selector_setup.route_s.name, v_s_route_selector_setup.route_s.namespace)
        # wait_before_test(1)
        # new_config = get_vs_nginx_template_conf(
        #     kube_apis.v1,
        #     v_s_route_setup.namespace,
