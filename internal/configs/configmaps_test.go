...
func TestParseConfigMapProxyNextUpstream(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:        "valid single value input",
			value:       "error",
			expected:    "error",
			description: "should parse single value input correctly",
		},
		{
			name:        "valid multiple value input",
			value:       "error timeout http_502",
			expected:    "error timeout http_502",
			description: "should parse multiple value input correctly",
		},
	}
	nginxPlus := false
	hasAppProtect := false
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.MainProxyNextUpstream != tt.expected {
				t.Errorf("ParseConfigMap() for %s returned MainProxyNextUpstream=%v, expected %v",
					tt.description, result.MainProxyNextUpstream, tt.expected)
			}
		})
	}
}

func TestParseConfigMapProxyNextUpstreamTimeoutValid(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			value:       "10m",
			expected:    "10m",
			description: "should accept input time as valid",
		},
		{
			name:        "valid zero input",
			value:       "50",
			expected:    "50s",
			description: "should accept zero as valid time",
		},
		{
			name:        "valid zero input",
			value:       "0",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cm := &v1.ConfigMap{
				...
				Data: map[string]string{
					"proxy-next-upstream-timeout": tt.value,
				},
			}
			result, configOk := ParseConfigMap(
				context.Background(),
			...
			if result.MainProxyNextUpstreamTimeout != tt.expected {
				t.Errorf("ParseConfigMap() for %s returned MainProxyNextUpstreamTimeout=%v, expected %v",
					tt.description, result.MainProxyNextUpstreamTimeout, tt.expected)
			}
		})
	}
}

func TestParseConfigMapProxyNextUpstreamTimeoutInvalid(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			value:       "abc",
			description: "should not accept non-numeric characters as valid",
		},
		{
			name:        "invalid negative number input",
			value:       "-123",
			description: "should not accept negative number as valid",
		},
		{
			name:        "invalid empty string input",
			value:       "",
			description: "should not accept empty string as valid",
		},
	}
	nginxPlus := false
	hasAppProtect := false
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.MainProxyNextUpstreamTimeout != "" {
				t.Errorf(`%s returned MainClientBodyBufferSize=%q, expected 0`,
					tt.name, result.MainClientBodyBufferSize)
			}
		})
	}
...
func TestParseConfigMapProxyNextUpstreamTriesValid(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			value:       "123",
			expected:    123,
			description: "should accept number input as valid",
		},
		{
			name:        "valid zero input",
			value:       "0",
			expected:    0,
			description: "should accept zero input as valid",
		},
	}
	nginxPlus := false
	hasAppProtect := false
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cm := &v1.ConfigMap{
				...
				Data: map[string]string{
					"proxy-next-upstream-tries": tt.value,
				},
			}
			result, configOk := ParseConfigMap(
				context.Background(),
			...
			if result.MainProxyNextUpstreamTries != tt.expected {
				t.Errorf("ParseConfigMap() for %s returned MainProxyNextUpstreamTries=%v, expected %v",
					tt.description, result.MainProxyNextUpstreamTries, tt.expected)
			}
		})
	}
}

func TestParseConfigMapProxyNextUpstreamTriesInvalid(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			name:        "invalid character input",
			value:       "abc",
			description: "should not accept input as valid",
		},
		{
			name:        "invalid negative number input",
			value:       "-123",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.MainProxyNextUpstreamTries != 0 {
				t.Errorf(`%s returned MainClientBodyBufferSize=%q, expected 0`,
					tt.name, result.MainProxyNextUpstreamTries)
			}
		})
	}
}
