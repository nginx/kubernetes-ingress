...
func TestMakeProxyBuffersAndProxyBufferSize(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		...
		{
			name:            "Large buffer-size corrected",
			proxyBuffers:    "8 1k",
			proxyBufferSize: "2m",
			expectedOutput:  "proxy_buffers 4 2m;\n\t\tproxy_buffer_size 2m;",
		},
		{
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got := makeProxyBuffersAndProxyBufferSize(tc.proxyBuffers, tc.proxyBufferSize)

			if got != tc.expectedOutput {
				t.Errorf("Input: buffers=%q, bufferSize=%q\nGot:      %q\nExpected: %q",
					tc.proxyBuffers, tc.proxyBufferSize, got, tc.expectedOutput)
			}
		})
	}
}

func TestCorrectProxyBuffers(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
	...
	}{
		{
			...
			inputBuffers:       "8 1k",
			inputBufferSize:    "2m",
			expectedBuffers:    "4 2m",
			expectedBufferSize: "2m",
		},
		{
			name:               "Correct configuration unchanged",
			inputBuffers:       "8 4k",
			inputBufferSize:    "4k",
			expectedBuffers:    "8 4k",
			expectedBufferSize: "4k",
		},
		{
			name:               "Empty inputs",
			inputBuffers:       "",
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			gotBuffers, gotBufferSize := correctProxyBuffers(tc.inputBuffers, tc.inputBufferSize)

			if gotBuffers != tc.expectedBuffers || gotBufferSize != tc.expectedBufferSize {
				t.Errorf("Input: buffers=%q, bufferSize=%q\nGot:      buffers=%q, bufferSize=%q\nExpected: buffers=%q, bufferSize=%q",
					tc.inputBuffers, tc.inputBufferSize, gotBuffers, gotBufferSize, tc.expectedBuffers, tc.expectedBufferSize)
			}
		})
	}
}
