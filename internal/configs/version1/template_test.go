...
// ingressCfgWithBasicAuthAndACMEChallenge tests that ACME challenge paths
// bypass basic auth when basic auth is enabled at server level.
var ingressCfgWithBasicAuthAndACMEChallenge = IngressNginxConfig{
	Servers: []Server{
		{
			Name:         "test.example.com",
			ServerTokens: "off",
...
func assertACMELocationsDisableBasicAuth(t *testing.T, bufString string, acmeLocations []string, contextMsg string) {
	...
	for _, acmeLoc := range acmeLocations {
		acmeIdx := strings.Index(bufString, acmeLoc)
		if acmeIdx == -1 {
			t.Fatalf("want ACME challenge location %q in generated config%s", acmeLoc, contextMsg)
		}
		...
		if authOffIdx == -1 {
			t.Errorf("want auth_basic off in ACME challenge location %q%s", acmeLoc, contextMsg)
			continue
		}

		proxyPassIdx := strings.Index(locationBlock, "proxy_pass")
		if proxyPassIdx != -1 && authOffIdx > proxyPassIdx {
			t.Errorf("auth_basic off should appear before proxy_pass in ACME challenge location %q%s", acmeLoc, contextMsg)
		}
	}
}
...
func TestExecuteTemplate_ForIngressForNGINXWithBasicAuthACMEChallenge(t *testing.T) {
	...
	err := tmpl.Execute(buf, ingressCfgWithBasicAuthAndACMEChallenge)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Server-level basic auth should be present
	if !strings.Contains(bufString, `auth_basic "Protected Area"`) {
		t.Error("want server-level auth_basic in generated config")
	}
	if !strings.Contains(bufString, "auth_basic_user_file /etc/nginx/secrets/htpasswd") {
		t.Error("want server-level auth_basic_user_file in generated config")
	}

	assertACMELocationsDisableBasicAuth(t, bufString, []string{
...
func TestExecuteTemplate_ForIngressForNGINXWithLocationBasicAuthACMEChallenge(t *testing.T) {
	...
	assertACMELocationsDisableBasicAuth(t, bufString, []string{
		...
		`location /.well-known/acme-challenge`,
		`location = /.well-known/acme-challenge/`,
	}, " when location-level basic auth is configured")

	snaps.MatchSnapshot(t, bufString)
	t.Log(bufString)
}

func assertACMEChallengeLocationsDisableBasicAuth(t *testing.T, bufString string, errSuffix string) {
	t.Helper()

	acmeLocations := []struct {
		name    string
		pattern string
	}{
		{
			name:    "prefix ACME challenge location with trailing slash",
			pattern: `location\s+/\.well-known/acme-challenge/\s*\{`,
		},
		{
			name:    "prefix ACME challenge location without trailing slash",
			pattern: `location\s+/\.well-known/acme-challenge\s*\{`,
		},
		{
	...
	for _, acmeLocation := range acmeLocations {
		...
		locationIdx := re.FindStringIndex(bufString)
		if locationIdx == nil {
			t.Errorf("want %s in generated config", acmeLocation.name)
			continue
		}
		...
		proxyPassIdx := strings.Index(locationBlock, "proxy_pass")
		if proxyPassIdx == -1 {
			t.Errorf("want proxy_pass in %s", acmeLocation.name)
			continue
		}

		if authOffIdx > proxyPassIdx {
			t.Errorf("auth_basic off should appear before proxy_pass in %s%s", acmeLocation.name, errSuffix)
		}
	}
}

func TestExecuteTemplate_ForIngressForNGINXPlusWithLocationBasicAuthACMEChallenge(t *testing.T) {
	t.Parallel()
	...
	assertACMEChallengeLocationsDisableBasicAuth(t, bufString, " when location-level basic auth is configured")

	snaps.MatchSnapshot(t, bufString)
	t.Log(bufString)
}

func TestExecuteTemplate_ForIngressForNGINXPlusWithBasicAuthACMEChallenge(t *testing.T) {
	t.Parallel()
	...
	assertACMEChallengeLocationsDisableBasicAuth(t, bufString, "")
