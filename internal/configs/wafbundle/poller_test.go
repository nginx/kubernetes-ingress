...
func TestPoller_WritesBundle_OnUpdate(t *testing.T) {
	...
	mgr := NewPollerManager(ff, tmpDir, func(key string) { syncCalled <- key }, testLogger())
...
func TestPoller_NoSync_WhenUnchanged(t *testing.T) {
	...
	synced := atomic.Int32{}
	mgr := NewPollerManager(ff, t.TempDir(), func(_ string) { synced.Add(1) }, testLogger())
...
func TestPoller_StopPoller_CancelsGoroutine(t *testing.T) {
	...
	ff := &fakeFetcher{}
	ff.set(Result{Unchanged: true}, nil)
	mgr := NewPollerManager(ff, t.TempDir(), func(_ string) {}, testLogger())
	mgr.ReconcilePoller("ns/pol", []PollSource{{
