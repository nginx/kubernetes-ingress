	...
	for {
		...
		if badPath != "" {
			if resourceName, tracked := cm.batchFiles[badPath]; tracked {
				...
				cm.excludeConfig(badPath, resourceName, testErr)
				continue
			}
		...
		cm.excludeConfig(badFile, resourceName, testErr)
	}
}
...
func (cm *ConfigRollbackManager) excludeConfig(configPath, resourceName string, reason error) {
	if cm.batchShadows[configPath] {
		...
		if err := os.Remove(configPath); err != nil {
			nl.Warnf(cm.logger, "Failed to remove excluded config %s: %v", configPath, err)
		}
	}
