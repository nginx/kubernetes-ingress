...
//
// This is also used for offsets like buffer sizes with sizeUnitless.
type sizeUnit int

const (
	sizeUnitless sizeUnit = iota
	sizeKB
	sizeMB
	sizeGB
)
...
// SizeWithUnit represents a size value with an optional unit. It's used for
// handling proxy NGINX configuration values that accept sizes with units.
...
func NewSizeWithUnit(sizeStr string) (SizeWithUnit, error) {
	...
	if err != nil || num < 0 {
		return SizeWithUnit{}, fmt.Errorf("invalid size value, must be non-negative integer and less than 9,223,372,036,854,775,807: %s", sizeStr)
	}
