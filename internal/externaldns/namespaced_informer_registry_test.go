...
func TestSyncFnSkipsUnwatchedNamespace(t *testing.T) {
	...
	vs := &vsapi.VirtualServer{
		...
		Spec: vsapi.VirtualServerSpec{
			ExternalDNS: vsapi.ExternalDNS{Enable: true},
		},
		Status: vsapi.VirtualServerStatus{
			ExternalEndpoints: []vsapi.ExternalEndpoint{{IP: "10.10.10.20"}},
		},
	}

	fn := SyncFnFor(EventRecorder{}, nil, nsregistry.New[namespacedInformer]())

	if err := fn(context.Background(), vs); err != nil {
