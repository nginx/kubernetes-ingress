...
func TestApplyWeightOnlyVSChanges_RejectsUnexpectedChangeShapes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if lbc.applyWeightOnlyVSChanges(lbc.Logger, key, test.changes, updates) {
...
func TestApplyWeightOnlyVSRChanges_RejectsUnexpectedChangeShapes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if lbc.applyWeightOnlyVSRChanges(lbc.Logger, test.changes, vsrOld, vsrNew) {
...
func TestApplyWeightOnlyVSRChanges_EmptyChangeSet(t *testing.T) {
	...
	if !lbc.applyWeightOnlyVSRChanges(lbc.Logger, nil, vsrOld, vsrNew) {
...
// TestWeightOnlyBaselineForVSR covers every branch of the fast-lane decision
// directly, including the ones no sync-level fixture can distinguish.
func TestWeightOnlyBaselineForVSR(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			prev, got := lbc.weightOnlyBaselineForVSR(lbc.Logger, "default/coffee", cur)
			if got != test.want {
				t.Errorf("weightOnlyBaselineForVSR() = %v, want %v", got, test.want)
			}
			if got && prev == nil {
				t.Error("weightOnlyBaselineForVSR() returned true with a nil baseline")
			}
			if !got && prev != nil {
				t.Error("weightOnlyBaselineForVSR() returned false with a non-nil baseline")
			}
		})
	}
}
